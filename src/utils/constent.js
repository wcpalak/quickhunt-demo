...
import slackIntImg from "@/assets/slacklogo.svg";
import gitHubIntImg from "@/assets/githublogo.png";
import zapierIntImg from "@/assets/zapier.webp";
import HubSpotImg from "@/assets/HubSpotImg.png";
import JiraImg from "@/assets/JiraImg.png";
import UpgradePayment from "@/assets/UpgradePayment.png";
import utc from "dayjs/plugin/utc";
import relativeTime from "dayjs/plugin/relativeTime";
import mixpanel from "mixpanel-browser";
import { useEffect, useState } from "react";
import { useImagePreview } from "../components/Comman/ImagePreviewProvider";
dayjs.extend(utc);
dayjs.extend(relativeTime);

export { slackIntImg, gitHubIntImg, zapierIntImg, HubSpotImg, JiraImg, UpgradePayment };
...
// Utility function to preserve search parameters during navigation
export const preserveSearchParams = (path, currentSearch = window.location.search) => {
    return currentSearch ? `${path}${currentSearch}` : path;
};

// Utility function to preserve search parameters but remove UTM parameters for specific routes when token exists
export const preserveSearchParamsWithoutUTM = (path, currentSearch = window.location.search) => {
    if (!currentSearch) return path;
    
    const urlParams = new URLSearchParams(currentSearch);
    const hasToken = localStorage.getItem(TOKEN_KEY) || 
                    localStorage.getItem("token-verify-onboard") || 
                    localStorage.getItem("token-verify-2fa");
    
    // Routes where UTM parameters should be removed when token exists
    const protectedRoutes = ['/onboarding', '/dashboard', '/verify-2fa'];
    const isProtectedRoute = protectedRoutes.some(route => path.includes(route));
    
    if (hasToken && isProtectedRoute) {
        // Remove UTM parameters
        const utmParams = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content'];
        utmParams.forEach(param => urlParams.delete(param));
...
export const BASE_URL_API = import.meta.env.VITE_APP_TYPE === 'local' ? import.meta.env.VITE_APP_API_URL_LOCAL :
    import.meta.env.VITE_APP_TYPE === 'dev' ? import.meta.env.VITE_APP_API_URL_DEV : import.meta.env.VITE_APP_API_URL_PROD;

export const WIDGET_DOMAIN = import.meta.env.VITE_APP_TYPE === 'prod' ? import.meta.env.VITE_APP_WIDGET_DOMAIN_PROD : import.meta.env.VITE_APP_WIDGET_DOMAIN_DEV;

export const ADMIN_DOMAIN = import.meta.env.VITE_APP_TYPE === 'prod' ? import.meta.env.VITE_APP_DOMAIN_PROD : import.meta.env.VITE_APP_DOMAIN_DEV;

export const DO_SPACES_ENDPOINT = import.meta.env.VITE_APP_TYPE === 'local' ? import.meta.env.VITE_APP_DO_SPACES_ENDPOINT_LOCAL : import.meta.env.VITE_APP_DO_SPACES_ENDPOINT;

export const TOKEN_KEY = import.meta.env.VITE_APP_TYPE === 'prod' ? import.meta.env.VITE_APP_TOKEN_KEY_PROD : import.meta.env.VITE_APP_TOKEN_KEY_DEV;

export const PROJECT_KEY = import.meta.env.VITE_APP_TYPE === 'prod' ? import.meta.env.VITE_APP_PROJECT_KEY_PROD : import.meta.env.VITE_APP_PROJECT_KEY_DEV;

export const imgPathUploadCut = import.meta.env.VITE_APP_TYPE === 'prod' ? `quickhunt/prod` : `quickhunt/stage`;

export const JIRA_CLIENT_ID = import.meta.env.VITE_APP_TYPE === 'prod' ? import.meta.env.VITE_APP_JIRA_CI_PROD : import.meta.env.VITE_APP_JIRA_CI_DEV;

export const PAYMENT_LIVE_ENV = import.meta.env.VITE_APP_TYPE === 'prod' ? import.meta.env.VITE_APP_PAYMENT_LIVE_ENV : import.meta.env.VITE_APP_PAYMENT_DEV_ENV;
export const PAYMENT_LIVE_TOKEN = import.meta.env.VITE_APP_TYPE === 'prod' ? import.meta.env.VITE_APP_PAYMENT_LIVE_TOKEN : import.meta.env.VITE_APP_PAYMENT_DEV_TOKEN;

export const USER_DETAILS_KEY = import.meta.env.VITE_APP_TYPE === 'prod' ? 'QH-USER-PROUD' : 'QH-USER-DEV';
export const MIX_PANEL_TOKEN = import.meta.env.VITE_APP_MIX_PANEL_TOKEN;

export const brandingURL = `https://quickhunt.app/?utm_source=poweredby&utm_medium=branding&utm_campaign=poweredby_quickhunt`;
...
export const getLSUserDetails = () => {
    const savedUser = localStorage.getItem(USER_DETAILS_KEY);
...
export const isContentEmpty = (content) => {
    ...
    const hasMediaTags = /<(img|video|iframe|svg|canvas|audio)[^>]*>/i.test(content);
    return strippedContent === "" && !hasMediaTags;
};
...
export const isEditorContentEmpty = (content) => {
    ...
    let strippedContent = content
        ...
        .trim();
    return strippedContent === '' && !hasMedia;
};
...
export const getTwoAuthTokenVerify = () => {
    return localStorage.getItem('token-verify-2fa');
}
...
export const urlRegExp = /^https:\/\/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-z]{2,63}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)$/;
...
export const getUserCountry = async () => {
    if (!countryPromise) {
        //old https://ipwho.is/
        countryPromise = fetch("https://api.ipinfo.io/lite/me?token=3f83174ea30d71")
            .then((res) => res.json())
            .then((data) => data?.country || "Unknown")
            .catch(() => "Unknown");
    }
    return countryPromise;
};

export const fileUploaderOnEditor = ({ uploadFolder, moduleName, isPathRemove = false, projectId }) => {
    return {
        uploadByFile: async (file) => {
            ...
            const res = await fetch(`${BASE_URL_API}/media/single`, {
                method: 'POST',
                headers: {
            ...
            if (!res.ok || !result?.data?.imageUrl || !result?.data?.uploadPath) {
                return {
                    success: false,
                    error: {
            ...
            const filename = imageUrl?.split?.('/')?.pop?.() || file.name;
            ...
            return {
                ...
                file: {
                    url: isPathRemove
                        ...
                        : `${DO_SPACES_ENDPOINT}/${result.data.uploadPath}/${result.data.imageUrl}`,
                    originalUrl: `${DO_SPACES_ENDPOINT}/${result.data.uploadPath}/${imageUrl}`,
...
export const loadCrisp = () => {
    ...
    window.$crisp = [];
    window.CRISP_WEBSITE_ID = "ebe13162-dc82-4f27-9b84-a1ab7833ab95";
...
export const getDateFormat = (date) => {
    const now = dayjs();
    const localDatetime = dayjs.utc(date).local();
    const NewDate = localDatetime.isValid() ? localDatetime : dayjs();
    ...
    if (diffInSeconds < 30) {
        ...
        return NewDate.format("DD MMM"); // e.g., "29 Apr"
    } else {
        return NewDate.format("DD MMM, YYYY"); // e.g., "29 Apr, 2025"
    }
};
...
export const IN_APP_WIDGET_LIMITS = {
    0: 2,       // Free
    1: 10,       // Starter
    2: 20,      // Growth
    3: Infinity // Premium
...
export const restoreImagePaths = (html, descriptionImages) => {
    if (!html || !descriptionImages || !Array.isArray(descriptionImages))
        return html;
    const parser = new DOMParser();
    const doc = parser.parseFromString(html, "text/html");
    const images = doc.getElementsByTagName("img");
    for (let img of images) {
        const imageKey = img.getAttribute("src");
        const matchingImage = descriptionImages.find((imgObj) => imgObj.key === imageKey);
        if (matchingImage) {
            img.src = `${DO_SPACES_ENDPOINT}/${matchingImage.path}`;
        }
    }
    return doc.body.innerHTML;
};

export const useImageClickOpenold = (ref, dependency) => {
    useEffect(() => {
  ...
  export const useImageClickOpen = (ref, dependency) => {
    ...
    useEffect(() => {
      ...
      const imgList = Array.from(imgs).map((img) => img.src);
...
export const identifyUser = (userId, userProperties = {}) => {
    ...
    if (typeof window !== "undefined" && mixpanel && import.meta.env.VITE_APP_TYPE === 'prod') {
        if (!userId) {
            console.warn("Mixpanel: userId is required for identification.");
            return;
        }
        ...
        console.error("Mixpanel is not available on the window object.");
    }
};
...
export const resetMixpanel = () => {
    if (typeof window !== "undefined" && mixpanel) {
        try {
            if (typeof mixpanel.reset === "function") {
                mixpanel.reset();
            } else if (typeof mixpanel.disable === "function") {
                mixpanel.disable();
            }
        } catch (error) {
...
export const tourSteps = [
    {
        page: "/feedback",
        selector: "#btn-feedback-create",
        title: "Organize User Feedback",
        content: "Collect and manage user feedback, track upvotes, and update feedback statuses with ease.",
    },
    {
        page: "/roadmap",
        selector: "#roadmap-section",
        title: "Build Your Roadmap",
        content: "Turn customer feedback into a clear roadmap — organize by category, manage progress, and share what’s coming with your users",
    },
    {
        page: "/changelog",
        selector: "#btn-changelog-create",
        extraSelectors: [".sidebar-changelog"],
        title: "Share What’s New",
        content: "Share product updates with users through a clear changelog — keep them informed about new features, improvements, and fixes.",
    },
    {
        page: window.innerWidth <= 1279 ? "/widget" : "/widget/type",
        selector: "#btn-widget-create",
        extraSelectors: [".sidebar-widget"],
        title: "Collect Feedback with Widgets",
        content: "Create your first widget, add it to your product, and start collecting feedback directly from your users.",
    },
    {
        page: window.innerWidth <= 1279 ? "/app-message" : "/app-message/type",
        selector: "#btn-app-message-create",
        extraSelectors: [".sidebar-app-message"],
        title: "Engage with Messages",
        content: "Add messages to your project to reach users, run surveys, share updates, and create checklists that guide them through your project.",
    },
    {
        page: "/settings/general-settings",
        selector: "#general-settings-section",
        extraSelectors: [".sidebar-settings"],
        title: "Customize Your Feedback Portal",
        content: "Easily adjust the visibility of tabs, customize titles and descriptions, and style the portal with your own colors.",
    },
    {
        page: "/dashboard",
        selector: "#preview-icon-btn",
        title: "View Live Feedback Portal",
        content: "Explore your public feedback portal — where users can share ideas, follow your roadmap, read changelogs, and access helpful docs.",
    },
];
